...
interface TagsInputContextProps {
  value: string[]; // المصفوفة التي تحتوي على القيم
  onValueChange: (value: string[]) => void; // الدالة لتغيير القيم
  inputValue: string; // القيمة المدخلة في الحقل النصي
  setInputValue: Dispatch<SetStateAction<string>>; // دالة لتحديث قيمة النص المدخل
  activeIndex: number; // المؤشر النشط في القائمة
  setActiveIndex: Dispatch<SetStateAction<number>>; // دالة لتحديث المؤشر النشط
}
...
export const TagsInput = forwardRef<HTMLDivElement, TagsInputProps>(
  ...
  (e: ClipboardEvent<HTMLInputElement>) => {
    ...
    setInputValue(text); // فقط تعبئة الحقل
  },
  [],
);
